 ...
 */
 ...
 // validation middleware checks to see if req.body meets the requirements
 const hasValidProperties = (req, res, next) => {
   const { data = {} } = req.body;
 
   // checks to see if the values in ValidProperties are included in the data. if any of the values are not included -> status and error message.
   VALID_REQUIRED_PROPERTIES.forEach((property) => {
     ...
     // checks to make sure that the value for people is a number otherwise -> status and error message is returned.
     ...
     // If reservation date returned from the database does not match the reg exp -> error and status message.
     if (
     ...
     // If reservation time returned from the database does not match the reg exp -> error and status message.
     if (
